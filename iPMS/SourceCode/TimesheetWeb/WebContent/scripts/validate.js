...
function isValidate(strDate) {
	...
	if (strDate.length != 8) err=1;
	b = strDate.substring(0, 2); // month
	c = strDate.substring(2, 3); // '/'
	d = strDate.substring(3, 5); // day
	e = strDate.substring(5, 6); // '/'
	f = strDate.substring(6, 8); // year
	if (b<1 || b>12) err = 1;
	if (c != '/') err = 1;
	if (d<1 || d>31) err = 1;
	if (e != '/') err = 1;
...
function CompareValue(strDate1, strDate2) {
 ...
 if (strDate1.length > 0 && strDate1.length > 0 ) {
	m1 = strDate1.substring(0, 2); // month1
	d1 = strDate1.substring(3, 5); // day1
	y1 = strDate1.substring(6, 8); // year1

	m2 = strDate2.substring(0, 2); // month2
	d2 = strDate2.substring(3, 5); // day2
	y2 = strDate2.substring(6, 8); // year2
