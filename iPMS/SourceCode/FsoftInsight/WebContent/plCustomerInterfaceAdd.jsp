...
<TITLE>plCustomerInterfaceAdd.jsp</TITLE>
...
%>
<BODY onLoad="loadPrjMenu();document.frmCustomerInterfaceAdd.interface_department[0].focus();" class="BD">

<form name ="frmCustomerInterfaceAdd" method= "post" action = "Fms1Servlet#CustomerInterfaces">
<P class="TITLE"> <%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.AddNewCustomerInterfaces")%> </P>
<% 	
	if ("1".equals(strErr)) {
%>		
...
<TABLE cellspacing="1" class="Table">
	<CAPTION class="TableCaption"> <%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.CustomerInterfaces")%> </CAPTION>
	<THEAD>
		<TR class="ColumnLabel">
			<TD width = "24" align = "center"> # </TD>
			<TD><%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.Department")%>* </TD>
			<TD>
				<%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.ContactPerson")%>* <BR>
				<%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.ContactPersonNote")%>
			</TD>
			<TD> 
				<%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.ContactAddress")%>* <BR>
				<%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.ContactAddressNote")%>
			</TD>
			<TD><%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.ContactAccount")%></TD>
			<TD><%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.Responsibility")%>* </TD>			
		</TR>
	</THEAD>
	<TBODY>
<%
...
%>
		<TR id="interface_row<%=row + 1%>" class="<%=rowStyle%>" valign="top">
			...
			<TD height="57">
			 <TEXTAREA rows="4" cols = "23" name="interface_department"><%=isOver ? ConvertString.toHtml(interfaceInfo.department):""%></TEXTAREA> 
			</TD>
			<TD height="57">
...
<p id="interface_addMoreLink"><a href="javascript:addMoreRow()"> <%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.AddMoreInterfaces")%> </a></p>
<BR>
<input type ="hidden" name = "reqType" value = ""/>
<INPUT type="button" name="plOK" value="<%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.OK")%>" class="BUTTON" onclick="addSubmit();">
<INPUT type="button" name="plCancel" value="<%=languageChoose.getMessage("fi.jsp.plCustomerInterfaceAdd.Cancel")%>" class="BUTTON" onclick="doCancel();">
</form>
...
function addSubmit(){	
	if (checkValid()) {
		frmCustomerInterfaceAdd.reqType.value=<%=Constants.PL_INTERFACE_CUSTOMER_ADD%>;
		frmCustomerInterfaceAdd.submit();
	} else return false;	
}

function doCancel(){
	frmCustomerInterfaceAdd.reqType.value =<%=Constants.PL_STRUCTURE_GET_PAGE%>;
	frmCustomerInterfaceAdd.submit();
}
...
%>
		...
		frmCustomerInterfaceAdd.strAccountName[0].focus();
<%
...
function checkValid(){
	...
	function onCheckAccount(rowCheck,event){
		if (trim(frmCustomerInterfaceAdd.strAccountName[rowCheck].value) == ""){
			window.alert("<%=languageChoose.getMessage("fi.jsp.woTeamAdd.YouMustInputUserAccount")%>");
			frmCustomerInterfaceAdd.strAccountName[rowCheck].focus();
			return ;
		}
        ...
        var url = "Fms1Servlet?reqType=<%=Constants.FILLTER_USER%>" + "&Account=" + frmCustomerInterfaceAdd.strAccountName[rowCheck].value + "&Type=" + rdAccountName;
		javascript:ajax_showOptions(document.forms['frmCustomerInterfaceAdd'].strAccountName[rowCheck], url, '', event);
	}
