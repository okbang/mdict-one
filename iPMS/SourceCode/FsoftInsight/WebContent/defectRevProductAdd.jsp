...
<TITLE>defectRevProductAdd.jsp</TITLE>
...
<%	
	...
	Vector vErr = (Vector) request.getAttribute("ErrDefectRevProductList");
	if (vErr != null) {
		isOver = true;
		nRow = vErr.size();
		request.removeAttribute("ErrDefectRevProductList");
	}
...
<form name ="frmDefectRevProductAdd" method= "post" action = "Fms1Servlet">
<P class="TITLE"> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.DefectAddProductForReview")%> </P>
<% 	
	if ("1".equals(strErr)) {
%>		
...
<TABLE cellspacing="1" class="Table">
	<CAPTION class="TableCaption"> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.ReviewProducts")%> </CAPTION>
	<THEAD>
		<TR class="ColumnLabel">
			<TD width="24" align = "center">#</TD>
			<TD> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.Process")%> </TD>
			<TD> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.Product")%> </TD>
			<TD> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.PlannedFoundByReview")%></TD>			
		</TR>
	</THEAD>
	<TBODY>
<%
		...
		<TR id="_row<%=row + 1%>" class="<%=rowStyle%>" valign="top">
			...
			<TD>
				<SELECT name="process" class="COMBO" onchange="SelectProcess('<%=row%>');" style="width:100px">
					...
					<OPTION value = "1"> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.Requirement")%> </OPTION>
					<OPTION value = "2"> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.Design")%> </OPTION>
					<OPTION value = "3"> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.Coding")%> </OPTION>
					<OPTION value = "4"> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.Other")%> </OPTION>
				</SELECT>
			</TD>
			<TD>
...
<p id="addMoreLink"><a href="javascript:addMoreRow()"> <%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.AddMoreReviewProduct")%> </a></p>
<BR>
<input type ="hidden" name = "reqType"/>
<input type ="hidden" name = "fromPage" value = "<%=fromPage%>"/>
<INPUT type="button" name="plOK" value="<%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.OK")%>" class="BUTTON" onclick="addSubmit();">
<INPUT type="button" name="plCancel" value="<%=languageChoose.getMessage("fi.jsp.defectRevProductAdd.Cancel")%>" class="BUTTON" onclick="<%="defect".equals(fromPage) ? "jumpURL('DefectView.jsp');" : "jumpURL('qualityObjective.jsp');"%>">
</form>
...
function addSubmit(){	
	if (checkValid()) {
		frmDefectRevProductAdd.reqType.value=<%=Constants.DEFECT_REV_PRODUCT_ADD%>;
		frmDefectRevProductAdd.submit();
	} else return false;	
}
