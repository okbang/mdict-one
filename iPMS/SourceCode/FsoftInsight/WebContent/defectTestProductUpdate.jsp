...
<TITLE>defectTestProductUpdate.jsp</TITLE>
...
<form name ="frmdefectTestProductUpdate" method= "post" action = "Fms1Servlet">
<P class="TITLE"> <%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.DefectUpdateProductForReview")%> </P>
<% 	
	if ("1".equals(strErr)) {
%>		
...
<TABLE cellspacing="1" class="Table" width="95%">
	<CAPTION class="TableCaption"> <%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.ReviewProducts")%> </CAPTION>
	<THEAD>
		<TR class="ColumnLabel">
			<TD width="3%" align = "center">#</TD>
			<TD width="20%"> <%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.Process")%> </TD>
			<TD width="50%"> <%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.Product")%> </TD>
			<TD width="22%"> <%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.PlannedFoundByReview")%></TD>			
		</TR>
	</THEAD>
	<TBODY>
<%
		...
		<TR id="_row<%=row + 1%>" class="<%=rowStyle%>" valign="top">
			...
			<TD>				
				<% switch (moduleInfo.processType) {
					case 1: %>
					<%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.Requirement")%>
				<%  break;
					case 2: %>				
					<%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.Design")%>
				<%  break;
					case 3: %>	
					<%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.Coding")%>
				<%  break;
					case 4: %>	
					<%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.Other")%>
				<% 	break;
					} %>				
			</TD>
			<TD><%=moduleInfo.name%></TD>
			<TD  align = "center">
			 <INPUT type ="text" size = "18" maxlength ="18" name="planSize" class="SmallTextbox" value="<%= (Double.isNaN(moduleInfo.newPlanSizeTest)) ? "" :moduleInfo.newPlanSizeTest+""%>"/>
			</TD>
		</TR>
<%	
	}
...
<INPUT type="button" name="plOK" value="<%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.OK")%>" class="BUTTON" onclick="addSubmit();">
<INPUT type="button" name="plCancel" value="<%=languageChoose.getMessage("fi.jsp.defectTestProductUpdate.Cancel")%>" class="BUTTON" onclick="<%="defect".equals(fromPage) ? "jumpURL('DefectView.jsp');" : "jumpURL('qualityObjective.jsp');"%>">
</form>
...
function addSubmit(){	
	if (checkValid()) {
		frmdefectTestProductUpdate.reqType.value=<%=Constants.DEFECT_TEST_PRODUCT_UPDATE%>;
		frmdefectTestProductUpdate.submit();
	} else return false;	
}
